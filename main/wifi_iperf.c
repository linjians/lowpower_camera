...
static void iperf_report_task(void *arg)
{
    ...
    while (!s_iperf_ctrl.finish) {
        ...
        printf("%4ld-%4ld sec       %.2f Mbits/sec\n", cur, cur + interval,
            actual_bandwidth);
        cur += interval;
        ...
        if (cur >= time) {
            printf("%4d-%4ld sec       %.2f Mbits/sec\n", 0, time,
                average);
            break;
        }
    }
...
static uint32_t wifi_get_local_ip(void)
{
    ...
    esp_netif_get_ip_info(wifi_get_Apnetif(), &ip_info);
