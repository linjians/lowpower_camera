...
typedef struct mdMisc {
    ...
    button_handle_t btn;            ///< Button handler
...
static void button_single_click_cb(void *arg, void *priv)
{
    ...
    if (camera_snapshot(SNAP_BUTTON, 1) == ESP_OK) {
...
static void button_double_click_cb(void *arg, void *priv)
{
    ...
    // led_indicator_stop(led_indicator_get_handle(STATUS_LED_IO), BLINK_RECONNECTING);
    // led_indicator_start(led_indicator_get_handle(STATUS_LED_IO), BLINK_PROVISIONED);
    storage_show_file();
    system_show_meminfo();
    // storage_sd_check();
}
...
static void button_start()
{
    ...
    g_misc.btn = iot_button_create(&cfg);
    iot_button_register_cb(g_misc.btn, BUTTON_PRESS_DOWN, button_press_down_cb, NULL);
    iot_button_register_cb(g_misc.btn, BUTTON_PRESS_UP, button_press_up_cb, NULL);
    iot_button_register_cb(g_misc.btn, BUTTON_PRESS_REPEAT, button_press_repeat_cb, NULL);
    iot_button_register_cb(g_misc.btn, BUTTON_SINGLE_CLICK, button_single_click_cb, NULL);
    iot_button_register_cb(g_misc.btn, BUTTON_DOUBLE_CLICK, button_double_click_cb, NULL);
    iot_button_register_cb(g_misc.btn, BUTTON_LONG_PRESS_START, button_long_press_start_cb, NULL);
    iot_button_register_cb(g_misc.btn, BUTTON_LONG_PRESS_HOLD, button_long_press_hold_cb, NULL);
}

static void button_stop()
{
    iot_button_delete(g_misc.btn);
}
