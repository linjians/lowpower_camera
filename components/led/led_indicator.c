 ...
 *
 */
static const blink_step_t snapshot[] = {
...
static SLIST_HEAD(_led_indicator_head_t,
...
led_indicator_handle_t led_indicator_create(int io_num, const led_indicator_config_t *config)
{
    ...
    switch (p_led_indicator->mode) {
        case LED_GPIO_MODE: {      /**< blink with max brightness*/
            ...
            p_led_indicator->h_timer = xTimerCreate(timmer_name, (pdMS_TO_TICKS(100)), pdFALSE, (void *)p_led_indicator,
                                                    _blink_list_runner);
...
{
    LED_INDICATOR_CHECK(handle != NULL && blink_type >= 0 &&
                        blink_type < BLINK_MAX, "invalid p_handle", ESP_ERR_INVALID_ARG);
