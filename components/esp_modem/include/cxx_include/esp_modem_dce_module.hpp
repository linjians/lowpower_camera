...
public:
    ...
    bool set_mode(modem_mode mode) override
    {
        if (mode == modem_mode::DATA_MODE) {
            ESP_LOGI("esp_modem_dce_module", "DATA_MODE");
            int retry = 0;
            while (retry++ < 20) {
                ESP_LOGI("esp_modem_dce_module", "DATA_MODE retry %d", retry);
                if (set_data_mode() == command_result::OK) {
                    return true;
                }
                if (resume_data_mode() == command_result::OK) {
                    return true;
                }
                Task::Delay(1000);
            }
            ...
            ESP_LOGI("esp_modem_dce_module", "COMMAND_MODE");
            ...
            ESP_LOGI("esp_modem_dce_module", "CMUX_MODE");
            int retry = 0;
            while (retry++ < 5) {
                ESP_LOGI("esp_modem_dce_module", "CMUX_MODE retry %d", retry);
                if (set_cmux() == command_result::OK) {
                    return true;
                }
                Task::Delay(1000);
            }
...
/**
 * @brief Specific definition of the EC800E module
 */
class EC800E: public GenericModule {
    using GenericModule::GenericModule;
public:
    command_result set_data_mode() override;
};
