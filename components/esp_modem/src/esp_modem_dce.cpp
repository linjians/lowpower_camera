...
{
    ESP_LOGI("esp_modem_dce", "enter_data");
    ESP_LOGI("esp_modem_dce", "device.setup_data_mode()");
    if (!device.setup_data_mode()) {
        ESP_LOGE("esp_modem_dce", "enter_data: !device.setup_data_mode()");
        return false;
    }
    ESP_LOGI("esp_modem_dce", "device.set_mode(modem_mode::DATA_MODE)");
    if (!device.set_mode(modem_mode::DATA_MODE)) {
        ESP_LOGE("esp_modem_dce", "enter_data: !device.set_mode(modem_mode::DATA_MODE)");
        return false;
    }
    ESP_LOGI("esp_modem_dce", "dte.set_mode(modem_mode::DATA_MODE)");
    if (!dte.set_mode(modem_mode::DATA_MODE)) {
        ESP_LOGE("esp_modem_dce", "enter_data: !dte.set_mode(modem_mode::DATA_MODE)");
        return false;
    }
    netif.start();
    return true;
...
bool DCE_Mode::set_unsafe(DTE *dte, ModuleIf *device, Netif &netif, modem_mode m)
{
    ...
    case modem_mode::CMUX_MODE:
        if (mode == modem_mode::DATA_MODE || mode == modem_mode::CMUX_MODE || mode >= modem_mode::CMUX_MANUAL_MODE) {
            ESP_LOGE("esp-modem", "CMUX_MODE: mode == modem_mode::DATA_MODE || mode == modem_mode::CMUX_MODE || mode >= modem_mode::CMUX_MANUAL_MODE");
            return false;
        }
        ...
        if (!dte->set_mode(modem_mode::CMUX_MODE)) {
            ESP_LOGE("esp-modem", "CMUX_MODE: !dte->set_mode(modem_mode::CMUX_MODE)");
            return false;
        }
